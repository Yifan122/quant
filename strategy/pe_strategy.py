...
def compose_signal(df, window=30):
    ...
    df['signal'] = np.where(df['pe_ratio'] < df['window_mean'] - df['window_std'], 1, 0)
    return df
...
if __name__ == '__main__':
    ...
    print(data)
    signal_df = data[data['signal'] == 1]

    # 绘制双Y轴曲线图
    fig = plt.figure()
    ax1 = fig.add_subplot(111)
    ax1.plot(data.index, data['pe_ratio'], label='pe_ratio')
    ax1.plot(signal_df['pe_ratio'], color='orange', marker='o', linestyle='None')
